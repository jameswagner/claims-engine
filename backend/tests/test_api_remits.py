...
def test_create_remit_returns_201(client, db):
    ...
    response = client.post(f"/claims/{claim.id}/remit", json=VALID_REMIT_BODY)
...
def test_create_remit_claim_not_found_returns_404(client, db):
    ...
    response = client.post(f"/claims/{uuid.uuid4()}/remit", json=VALID_REMIT_BODY)
...
def test_create_remit_unknown_code_accepted(client, db):
    ...
    response = client.post(f"/claims/{claim.id}/remit", json=body)
    assert response.status_code == 201
...
def test_create_remit_updates_claim_financials(client, db):
    ...
    client.post(f"/claims/{claim.id}/remit", json=VALID_REMIT_BODY)
...
def test_create_remit_missing_codes_returns_422(client, db):
    ...
    response = client.post(f"/claims/{uuid.uuid4()}/remit", json=body)
    assert response.status_code == 422
